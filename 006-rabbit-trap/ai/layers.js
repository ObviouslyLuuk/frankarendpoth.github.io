...
export class OutputLayer extends Layer {
    ...
    feedforward(input, labels) {
        ...
        let correct = get_max_value_index(labels)
        if (prediction == correct)
            score = 1

        this.time.Feedforward += Date.now() - t0
        return {output: this.activations, cost: cost, score: score}
    }
...
export class DenseLayer extends Layer {
    ...
    backprop() {
        ...
        let prev_a_nudges = []
        for (let j = 0; j < this.size; j++) {
            // let d_bias = sigmoid_prime(this.zs[j]) * a_nudges[j] // derivative of Cost with respect to the bias
            let d_bias = ReLU_prime(this.zs[j]) * a_nudges[j] // derivative of Cost with respect to the bias
            this.total_bias_nudges[j] += d_bias
